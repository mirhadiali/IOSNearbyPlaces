...
extension NearbyPlacesViewController: PlaceCellDelegate {
    ...
    func didClickCall(place: QPlace) {
        NearbyPlacesController.getPlaceDetails(place: place) { (place) in
            if let call = place.details?["formatted_phone_number"] as? String{
                let charsToRemove: Set<Character> = Set("()- ".characters)
                let number = String(call.characters.filter { !charsToRemove.contains($0) })
                print(number)
                if let url = URL(string: "tel://\(number)"),
                    UIApplication.shared.canOpenURL(url) {
                    UIApplication.shared.open(url)
                }
//this below trick is better but I just used above one because I don't want to remove + sign
//               let number = call.components(separatedBy: CharacterSet.decimalDigits.inverted).joined()
